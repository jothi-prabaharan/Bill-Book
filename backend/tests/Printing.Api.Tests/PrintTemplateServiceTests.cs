...
public sealed class PrintTemplateServiceTests
{
    ...
    private PrintTemplateService Service(PrintingDbContext db) => new(db, new PrintRenderer());
    ...
    public async Task Two_templates_of_one_document_type_keep_their_own_settings_and_content()
    {
        ...
        await using PrintingDbContext fresh = _postgres.CreateContext(customer, org);
    ...
    public async Task Concurrent_requests_to_set_a_default_leave_exactly_one()
    {
        ...
        await using (PrintingDbContext seed = _postgres.CreateContext(customer, org))
        {
    ...
    public async Task One_branch_cannot_see_another_branchs_template()
    {
        ...
        long id;
        await using (PrintingDbContext a = _postgres.CreateContext(customer, orgA))
        {
            id = (await Service(a).CreateAsync(Create("Branch A only"), Ct))
                .Value!.PrintTemplateId;
        }

        await using PrintingDbContext b = _postgres.CreateContext(customer, orgB);
    ...
    public async Task One_customer_cannot_see_another_customers_template()
    {
        ...
        long id;
        await using (PrintingDbContext a = _postgres.CreateContext(Guid.NewGuid(), orgA))
        {
            id = (await Service(a).CreateAsync(Create("Customer A only"), Ct))
                .Value!.PrintTemplateId;
        }

        await using PrintingDbContext b = _postgres.CreateContext(Guid.NewGuid(), orgB);
