...
/// <summary>Every Employee endpoint carries a guard, names its apps, and demands only seeded modules (TK-48).</summary>
public sealed class EndpointGuardTests
{
    private static System.Reflection.Assembly Service =>
        typeof(Employee.Api.Controllers.EmployeesController).Assembly;

    [Fact]
    public void Every_endpoint_carries_a_guard() =>
        Assert.Equal(string.Empty, string.Join(", ", EndpointGuardAudit.Unguarded(Service, "MeProfileController", "TeamController")));
