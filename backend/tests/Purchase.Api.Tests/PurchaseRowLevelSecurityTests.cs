...
/// <summary>
/// What the <c>pur</c> policies let through, asked as a role they bind.
///
...
public sealed class PurchaseRowLevelSecurityTests
{
    /// <summary>A role with no superuser and no BYPASSRLS, so policies bind it.</summary>
    private const string ProbeRole = "pur_rls_probe";

    private readonly PostgresFixture _postgres;

    public PurchaseRowLevelSecurityTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task With_no_tenant_set_queries_return_no_rows_and_do_not_throw()
    {
        ...
        Assert.Equal(0, await CountAsync(connection, "PurchaseOrders"));
    }
    ...
    private async Task<(Guid CustomerId, Guid OrgId)> SeedBranchAsync()
    {
        ...
        await using PurchaseDbContext db = _postgres.CreateContext(customerId, orgId);
    ...
    private async Task<NpgsqlConnection> OpenAsProbeAsync(Guid customerId, Guid orgId)
    {
        ...
        await ExecuteAsync(connection, $"""
            ...
            END
            $$;
            GRANT USAGE ON SCHEMA pur TO {ProbeRole};
            GRANT SELECT, INSERT ON ALL TABLES IN SCHEMA pur TO {ProbeRole};
            GRANT USAGE ON ALL SEQUENCES IN SCHEMA pur TO {ProbeRole};
            """);
    ...
    {
        await using NpgsqlCommand command = connection.CreateCommand();
        command.CommandText = $"SELECT count(*) FROM pur.\"{table}\"";
