...
public sealed class EmployeeServiceTests
{
    ...
    private sealed record Branch(HrmDbContext Db, long Department, long Designation, long Grade, long Location);

    private async Task<Branch> NewBranchAsync()
    {
        Guid customerId = Guid.NewGuid(), orgId = Guid.NewGuid();
        HrmDbContext db = _postgres.CreateContext(customerId, orgId);
        await new HrmSeeder(db).SeedForOrganizationAsync(orgId, default);
    ...
    private static EmployeeService Service(HrmDbContext db, ICallerPermissions caller) =>
    ...
    public async Task An_employee_is_created_with_family_nominees_and_bank_linked_to_a_user_and_listed_masked()
    {
        ...
        await using HrmDbContext _ = b.Db;
    ...
    public async Task Another_branch_cannot_read_the_employee()
    {
        ...
        await using HrmDbContext other = _postgres.CreateContext(Guid.NewGuid(), Guid.NewGuid());
...
public sealed class EmployeeRuleTests
{
    ...
    public void Every_refusal_has_a_sentence_that_names_no_figure()
    {
        ...
        {
            string sentence = Hrm.Api.Controllers.EmployeesController.Sentence(outcome);
