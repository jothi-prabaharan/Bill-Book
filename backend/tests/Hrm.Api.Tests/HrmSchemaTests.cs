...
public sealed class HrmSchemaTests
{
    private readonly PostgresFixture _postgres;

    public HrmSchemaTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task Every_org_scoped_entity_has_a_query_filter_and_xmin()
    {
        ...
        await using HrmDbContext db = _postgres.CreateContext(Guid.NewGuid(), Guid.NewGuid());
    ...
    public async Task Seeding_a_branch_twice_adds_nothing_the_second_time()
    {
        ...
        Dictionary<string, int> first = await new HrmSeeder(db).SeedForOrganizationAsync(orgId, default);
        Dictionary<string, int> second = await new HrmSeeder(db).SeedForOrganizationAsync(orgId, default);
    ...
    public async Task One_branch_never_sees_another_branchs_departments()
    {
        ...
        await using (HrmDbContext db = _postgres.CreateContext(customerId, mine))
        {
            db.Departments.Add(new Department { Code = "MINE", Name = "Mine" });
            await db.SaveChangesAsync();
        }

        await using HrmDbContext other = _postgres.CreateContext(customerId, Guid.NewGuid());
