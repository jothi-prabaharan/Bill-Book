...
public sealed class AmcSchemaTests
{
    private readonly PostgresFixture _postgres;

    public AmcSchemaTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task Seeding_a_branch_twice_adds_nothing_the_second_time()
    {
        ...
        await using AmcDbContext db = _postgres.CreateContext(customerId, orgId);

        await new AmcSeeder().SeedForOrganizationAsync(orgId, default);
        Dictionary<string, int> second = await new AmcSeeder().SeedForOrganizationAsync(orgId, default);
