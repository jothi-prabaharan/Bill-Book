...
/// <summary>
/// What the <c>rpt</c> policies let through, asked as a role they bind.
///
...
public sealed class ReportingRowLevelSecurityTests
{
    /// <summary>A role with no superuser and no BYPASSRLS, so policies bind it.</summary>
    private const string ProbeRole = "rpt_rls_probe";

    private readonly PostgresFixture _postgres;

    public ReportingRowLevelSecurityTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task With_no_tenant_set_queries_return_no_rows_and_do_not_throw()
    {
        ...
        Assert.Equal(0, await CountAsync(connection, "Reports"));
    }
    ...
    /// <summary>
    /// The initial migration's policies compared OrgId with
    /// <c>current_setting('tenant.orgid')</c>, which nothing sets, so under a
    /// role that does not bypass RLS nobody could read their own reports. None
    /// may survive the replacement, and the specification tables stay exempt.
    /// </summary>
    [SkippableFact]
    public async Task The_broken_tenant_orgid_policies_are_gone_and_the_specification_stays_unscoped()
    {
        Skip.If(_postgres.SkipReason is not null, _postgres.SkipReason ?? string.Empty);
        ...
        command.CommandText = """
            ...
            WHERE schemaname = 'rpt'
              AND (policyname = 'TenantPolicy'
                   OR qual LIKE '%tenant.orgid%'
                   OR tablename IN ('ReportMasters', 'ReportColumns'))
            """;
        ...
        await using (NpgsqlDataReader reader = await command.ExecuteReaderAsync())
        {
            ...
            {
                offending.Add(reader.GetString(0));
            }
        }
    ...
    private async Task<(Guid CustomerId, Guid OrgId)> SeedBranchAsync()
    {
        ...
        await using ReportingDbContext db = _postgres.CreateContext(customerId, orgId);
    ...
    private async Task<NpgsqlConnection> OpenAsProbeAsync(Guid customerId, Guid orgId)
    {
        ...
        await ExecuteAsync(connection, $"""
            ...
            END
            $$;
            GRANT USAGE ON SCHEMA rpt TO {ProbeRole};
            GRANT SELECT, INSERT ON ALL TABLES IN SCHEMA rpt TO {ProbeRole};
            GRANT USAGE ON ALL SEQUENCES IN SCHEMA rpt TO {ProbeRole};
            """);
    ...
    {
        await using NpgsqlCommand command = connection.CreateCommand();
        command.CommandText = $"SELECT count(*) FROM rpt.\"{table}\"";
