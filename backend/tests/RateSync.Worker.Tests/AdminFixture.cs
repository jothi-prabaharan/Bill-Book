...
/// <summary>
/// A real PostgreSQL holding the master database, for the rate sync (TK-26).
/// A database of its own, <c>ratesync_tests</c>, rather than Master's
/// <c>admin_tests</c>: test assemblies run in parallel, and two suites migrating
/// one database at once race on its indexes.
///
...
/// <c>RATESYNC_TEST_DB</c> at one to run it. A server that answers and refuses is a
/// failure, never a skip — see <c>PostgresFixture</c>, which this mirrors.
/// </summary>
public sealed class AdminFixture : IAsyncLifetime
{
    private const string DefaultConnection =
        "Host=localhost;Port=5432;Database=ratesync_tests;Username=postgres;Password=123";

    public string? SkipReason { get; private set; }

    public string ConnectionString =>
        Environment.GetEnvironmentVariable("RATESYNC_TEST_DB") ?? DefaultConnection;
    ...
    public async Task InitializeAsync()
    {
        ...
        catch (Exception ex) when (IsUnreachable(ex))
        {
            SkipReason =
                ...
                + $"tests did not run. Set RATESYNC_TEST_DB to point at one. ({ex.GetType().Name})";
        }
    }
