...
public sealed class StudentSchemaTests
{
    private readonly PostgresFixture _postgres;

    public StudentSchemaTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task Every_org_scoped_entity_has_a_query_filter_and_xmin()
    {
        ...
        await using StudentDbContext db = _postgres.CreateContext(Guid.NewGuid(), Guid.NewGuid());
    ...
    public async Task Seeding_a_branch_twice_adds_nothing_the_second_time()
    {
        ...
        await using StudentDbContext db = _postgres.CreateContext(customerId, orgId);

        await new StudentSeeder(db).SeedForOrganizationAsync(orgId, default);
        Dictionary<string, int> second = await new StudentSeeder(db).SeedForOrganizationAsync(orgId, default);
