...
internal sealed class FakeContacts : IContactDirectory
{
    ...
    public Task<EnsureGuardianResponse> EnsureGuardianAsync(string displayName, string mobileNumber, string? email, CancellationToken ct) =>
        throw new NotSupportedException("Student never creates guardians.");
}
...
public sealed class StudentServiceTests
{
    ...
    private sealed record Branch(StudentDbContext Db, long Year, long SectionA, FakeContacts Contacts);

    private async Task<Branch> NewBranchAsync(int? capacity = null)
    {
        Guid customerId = Guid.NewGuid(), orgId = Guid.NewGuid();
        StudentDbContext db = _postgres.CreateContext(customerId, orgId);
    ...
    public async Task A_student_admitted_directly_is_numbered_enrolled_and_listed()
    {
        ...
        await using StudentDbContext _ = b.Db;

        StudentResult created = await Service(b).CreateAsync(Arun(b), default);

        Assert.Equal(StudentOutcome.Ok, created.Outcome);
    ...
    public async Task A_guardian_that_is_not_a_guardian_contact_is_refused_and_no_number_is_spent()
    {
        ...
        Assert.Equal(StudentOutcome.GuardianRule, (await Service(b).CreateAsync(request, default)).Outcome);
    ...
    public async Task An_unknown_or_inactive_guardian_is_refused()
    {
        ...
        Assert.Equal(StudentOutcome.GuardianRule, (await Service(b).CreateAsync(unknown, default)).Outcome);
        Assert.Equal(StudentOutcome.GuardianRule, (await Service(b).CreateAsync(inactive, default)).Outcome);
    }
    ...
    public async Task Master_being_down_is_unavailable_not_a_refusal()
    {
        ...
        Assert.Equal(StudentOutcome.Unavailable, (await Service(b).CreateAsync(Arun(b), default)).Outcome);
    }
    ...
    public async Task A_full_section_refuses_the_next_enrolment()
    {
        ...
        Assert.Equal(StudentOutcome.Ok, (await Service(b).CreateAsync(Arun(b, roll: 1), default)).Outcome);
        Assert.Equal(StudentOutcome.SectionFull, (await Service(b).CreateAsync(Arun(b, roll: 2), default)).Outcome);
    }
    ...
    public async Task A_student_is_enrolled_once_per_year_and_a_roll_number_once_per_section()
    {
        ...
        Assert.Equal(StudentOutcome.Duplicate,
    ...
    public async Task A_closed_year_takes_no_enrolment()
    {
        ...
        Assert.Equal(StudentOutcome.YearRule, (await Service(b).CreateAsync(Arun(b), default)).Outcome);
    }
    ...
    public async Task Admitting_the_same_application_twice_returns_one_student()
    {
        ...
        StudentResult first = await Service(b).CreateAsync(Arun(b), default, sourceApplicationId: 77);
        StudentResult second = await Service(b).CreateAsync(Arun(b, roll: 2), default, sourceApplicationId: 77);
    ...
    public async Task One_branch_never_sees_another_branchs_students()
    {
        ...
        await using StudentDbContext _a = mine.Db;
        await using StudentDbContext _b = theirs.Db;
