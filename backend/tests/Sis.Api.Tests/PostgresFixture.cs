...
/// Point <c>SIS_TEST_DB</c> at one to run it.
/// </summary>
public sealed class PostgresFixture : IAsyncLifetime
{
    private const string DefaultConnection =
        "Host=localhost;Port=5432;Database=sis_tests;Username=postgres;Password=123";

    public string? SkipReason { get; private set; }

    public string ConnectionString =>
        Environment.GetEnvironmentVariable("SIS_TEST_DB") ?? DefaultConnection;
    ...
    public async Task InitializeAsync()
    {
        try
        {
            ...
            await using SisDbContext db = CreateContext(Guid.NewGuid(), Guid.NewGuid());
        ...
        catch (Exception ex) when (IsUnreachable(ex))
        {
            SkipReason =
                ...
                + $"tests did not run. Set SIS_TEST_DB to point at one. ({ex.GetType().Name})";
        }
    }
    ...
    public SisDbContext CreateContext(Guid customerId, Guid orgId) =>
        new(
            new DbContextOptionsBuilder<SisDbContext>()
