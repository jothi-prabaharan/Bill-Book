...
public sealed class ExamServiceTests
{
    ...
    public async Task Marks_are_taken_only_while_the_exam_is_open_and_published_marks_can_be_read_back()
    {
        ...
        await using SisDbContext db = _postgres.CreateContext(Guid.NewGuid(), orgId);
        ...
        var student = new Sis.Entity.TableEntities.Student
        {
            AdmissionNo = "ADM-1", FirstName = "Arun", DateOfBirth = new DateOnly(2015, 2, 10), AdmissionDate = new DateOnly(2026, 6, 1),
        };
        db.Students.Add(student);
        await db.SaveChangesAsync();
        var enrolment = new Sis.Entity.TableEntities.Enrolment { StudentId = student.StudentId, AcademicYearId = year, SectionId = section, RollNo = 1 };
        ...
        Assert.Equal(SisOutcome.ExamState, (await exams.SaveMarksAsync(exam, marks, default)).Outcome);

        await exams.MoveAsync(exam, ExamStatus.MarksOpen, default);
        Assert.Equal(SisOutcome.Ok, (await exams.SaveMarksAsync(exam, marks, default)).Outcome);
        ...
        Assert.Equal(SisOutcome.ExamState, (await exams.SaveMarksAsync(exam, marks, default)).Outcome);
        Assert.Equal(SisOutcome.ExamState, (await exams.SaveAsync(exam, new SaveExamRequest
        {
