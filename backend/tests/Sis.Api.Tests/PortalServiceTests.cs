...
public sealed class PortalServiceTests
{
    ...
    private sealed record School(SisDbContext Db, long Year, long Section, long ExamSubject, long Exam, long Meera, long Arun);
    ...
    private async Task<School> NewSchoolAsync()
    {
        ...
        SisDbContext db = _postgres.CreateContext(Guid.NewGuid(), orgId);
        ...
        var meera = new Student { AdmissionNo = "ADM-1", FirstName = "Meera", LastName = "Raman", DateOfBirth = new DateOnly(2015, 2, 10), AdmissionDate = new DateOnly(2026, 6, 1) };
        var arun = new Student { AdmissionNo = "ADM-2", FirstName = "Arun", LastName = "Raman", DateOfBirth = new DateOnly(2017, 5, 3), AdmissionDate = new DateOnly(2026, 6, 1) };
    ...
    public async Task A_guardian_sees_their_children_with_class_and_section()
    {
        ...
        await using SisDbContext _ = s.Db;
