...
/// <summary>
/// A real PostgreSQL for the <c>wrk</c> suite (S6, TK-66): the RLS policies
...
/// Point <c>WORKORDER_TEST_DB</c> at one to run it.
/// </summary>
public sealed class PostgresFixture : IAsyncLifetime
{
    private const string DefaultConnection =
        "Host=localhost;Port=5432;Database=workorder_tests;Username=postgres;Password=123";

    public string? SkipReason { get; private set; }

    public string ConnectionString =>
        Environment.GetEnvironmentVariable("WORKORDER_TEST_DB") ?? DefaultConnection;
    ...
    public async Task InitializeAsync()
    {
        try
        {
            ...
            await using WorkOrderDbContext db = CreateContext(Guid.NewGuid(), Guid.NewGuid());
        ...
        catch (Exception ex) when (IsUnreachable(ex))
        {
            SkipReason =
                ...
                + $"tests did not run. Set WORKORDER_TEST_DB to point at one. ({ex.GetType().Name})";
        }
    }
    ...
    public WorkOrderDbContext CreateContext(Guid customerId, Guid orgId) =>
        new(
            new DbContextOptionsBuilder<WorkOrderDbContext>()
                .UseNpgsql(ConnectionString, o => o.MigrationsHistoryTable("__EFMigrationsHistory", "wrk"))
                .Options,
