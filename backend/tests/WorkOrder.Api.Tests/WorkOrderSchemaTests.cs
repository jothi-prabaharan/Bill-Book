...
/// <summary>What the wrk schema guarantees, asked of a database built from the migration (S6, TK-66).</summary>
[Collection(nameof(PostgresCollection))]
public sealed class WorkOrderSchemaTests
{
    private readonly PostgresFixture _postgres;

    public WorkOrderSchemaTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task Every_org_scoped_entity_has_a_query_filter_and_xmin()
    {
        ...
        await using WorkOrderDbContext db = _postgres.CreateContext(Guid.NewGuid(), Guid.NewGuid());
    ...
    /// <summary>Every wrk table is RLS-enabled, FORCEd and policed, from a clean migration.</summary>
    [SkippableFact]
    public async Task Row_level_security_is_enabled_forced_and_policied_on_every_table()
    {
        ...
        Assert.Equal(
            string.Empty,
            string.Join("; ", await BillBook.Tests.Shared.RlsAudit.UnprotectedAsync(db, "wrk", "__EFMigrationsHistory")));
    }

    /// <summary>The sal bug of 21 August, asked of wrk before it can happen: no shadow foreign key.</summary>
    [SkippableFact]
    ...
    public async Task Seeding_a_branch_twice_adds_nothing_the_second_time()
    {
        ...
        await using WorkOrderDbContext db = _postgres.CreateContext(customerId, orgId);

        await new WorkOrderSeeder(db).SeedForOrganizationAsync(orgId, default);
        Dictionary<string, int> second = await new WorkOrderSeeder(db).SeedForOrganizationAsync(orgId, default);
