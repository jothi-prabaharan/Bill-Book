...
/// <summary>What the amc schema guarantees, asked of a database built from the migration (S8, TK-68).</summary>
[Collection(nameof(PostgresCollection))]
public sealed class MaintenanceContractSchemaTests
{
    private readonly PostgresFixture _postgres;

    public MaintenanceContractSchemaTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task Every_org_scoped_entity_has_a_query_filter_and_xmin()
    {
        ...
        await using MaintenanceContractDbContext db = _postgres.CreateContext(Guid.NewGuid(), Guid.NewGuid());
    ...
    /// <summary>Every amc table is RLS-enabled, FORCEd and policed, from a clean migration.</summary>
    [SkippableFact]
    public async Task Row_level_security_is_enabled_forced_and_policied_on_every_table()
    {
        ...
        Assert.Equal(
            string.Empty,
            string.Join("; ", await BillBook.Tests.Shared.RlsAudit.UnprotectedAsync(db, "amc", "__EFMigrationsHistory")));
    }

    /// <summary>The sal bug of 21 August, asked of amc before it can happen: no shadow foreign key.</summary>
    [SkippableFact]
    ...
    public async Task Seeding_a_branch_twice_adds_nothing_the_second_time()
    {
        ...
        await using MaintenanceContractDbContext db = _postgres.CreateContext(customerId, orgId);

        await new MaintenanceContractSeeder().SeedForOrganizationAsync(orgId, default);
        Dictionary<string, int> second = await new MaintenanceContractSeeder().SeedForOrganizationAsync(orgId, default);
