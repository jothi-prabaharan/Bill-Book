...
/// <summary>What the fac schema guarantees, asked of a database built from the migration (S5, TK-65).</summary>
[Collection(nameof(PostgresCollection))]
public sealed class FacilitySchemaTests
{
    private readonly PostgresFixture _postgres;

    public FacilitySchemaTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    /// <summary>Every fac table is RLS-enabled, FORCEd and policed, from a clean migration.</summary>
    [SkippableFact]
    public async Task Row_level_security_is_enabled_forced_and_policied_on_every_table()
    {
        ...
        Assert.Equal(
            string.Empty,
            string.Join("; ", await BillBook.Tests.Shared.RlsAudit.UnprotectedAsync(db, "fac", "__EFMigrationsHistory")));
    }

    /// <summary>The sal bug of 21 August, asked of fac before it can happen: no shadow foreign key.</summary>
    [SkippableFact]
    ...
    public async Task Seeding_a_branch_twice_adds_nothing_the_second_time()
    {
        ...
        await using FacilityDbContext db = _postgres.CreateContext(customerId, orgId);

        await new FacilitySeeder().SeedForOrganizationAsync(orgId, default);
        Dictionary<string, int> second = await new FacilitySeeder().SeedForOrganizationAsync(orgId, default);
