...
/// <summary>What the adm schema guarantees, asked of a database built from the migration (S2, TK-62).</summary>
[Collection(nameof(PostgresCollection))]
public sealed class AdmissionSchemaTests
{
    private readonly PostgresFixture _postgres;

    public AdmissionSchemaTests(PostgresFixture postgres) => _postgres = postgres;
    ...
    public async Task Every_org_scoped_entity_has_a_query_filter_and_xmin()
    {
        ...
        await using AdmissionDbContext db = _postgres.CreateContext(Guid.NewGuid(), Guid.NewGuid());
    ...
    /// <summary>Every adm table is RLS-enabled, FORCEd and policed, from a clean migration.</summary>
    [SkippableFact]
    public async Task Row_level_security_is_enabled_forced_and_policied_on_every_table()
    {
        ...
        Assert.Equal(
            string.Empty,
            string.Join("; ", await BillBook.Tests.Shared.RlsAudit.UnprotectedAsync(db, "adm", "__EFMigrationsHistory")));
    }

    /// <summary>The sal bug of 21 August, asked of adm before it can happen: no shadow foreign key.</summary>
    [SkippableFact]
    ...
    public async Task Seeding_a_branch_twice_adds_nothing_the_second_time()
    {
        ...
        await using AdmissionDbContext db = _postgres.CreateContext(customerId, orgId);

        await new AdmissionSeeder(db).SeedForOrganizationAsync(orgId, default);
        Dictionary<string, int> second = await new AdmissionSeeder(db).SeedForOrganizationAsync(orgId, default);
