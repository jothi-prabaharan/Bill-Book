...
/// <summary>
/// Writes Student's master data for a branch (S1, TK-61). Called by Master's
...
public sealed class InternalSeedController : ControllerBase
{
    ...
    public async Task<IActionResult> SeedOrganization([FromBody] SeedOrganizationRequest request, CancellationToken ct)
    {
        ...
        {
            Seeded = await _services.GetRequiredService<StudentSeeder>().SeedForOrganizationAsync(request.OrgId, ct),
        };

        _log.LogInformation("Seeded Student for organization {OrgId}: {@Seeded}", request.OrgId, response.Seeded);
        return Ok(response);
    }
}
