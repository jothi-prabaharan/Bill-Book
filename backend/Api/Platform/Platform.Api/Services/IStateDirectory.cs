...
/// <summary>
/// States, read from Master. An organization holds an unenforced StateId and
/// needs the state's GST code to check its GSTIN's first two digits — the same
/// check Contacts runs, against the same master data, because getting it wrong
/// sends CGST + SGST where IGST belongs and nothing complains until filing.
/// </summary>
...
public sealed class MasterStateDirectory : IStateDirectory
{
    ...
    public async Task<string?> GetStateCodeAsync(int stateId, CancellationToken ct)
    {
        ...
        // costs nothing and keeps a branch save off the network.
    ...
    private sealed record StateResponse(
        int StateId, int CountryId, string StateCode, string StateName, bool IsActive);
}
