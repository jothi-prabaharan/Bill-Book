...
[InternalOnly]
[Route("internal/amc")]
public sealed class InternalMaintenanceContractsController : ControllerBase
{
    private readonly TenantContext _tenant;
    private readonly IServiceProvider _services;

    public InternalMaintenanceContractsController(TenantContext tenant, IServiceProvider services)
    {
        _tenant = tenant;
        _services = services;
    }
    ...
    public async Task<IActionResult> RenewalsDue([FromBody] AmcRenewalQuery request, CancellationToken ct)
    {
        ...
        return Ok(await _services.GetRequiredService<MaintenanceContractService>().RenewalsDueAsync(request.On, ct));
    }
}
