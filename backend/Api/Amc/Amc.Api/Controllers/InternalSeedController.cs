...
/// <summary>
/// Writes Amc's master data for a branch (S8, TK-68). Called by Master's
...
public sealed class InternalSeedController : ControllerBase
{
    ...
    public async Task<IActionResult> SeedOrganization([FromBody] SeedOrganizationRequest request, CancellationToken ct)
    {
        ...
        {
            Seeded = await _services.GetRequiredService<AmcSeeder>().SeedForOrganizationAsync(request.OrgId, ct),
        };

        _log.LogInformation("Seeded Amc for organization {OrgId}: {@Seeded}", request.OrgId, response.Seeded);
        return Ok(response);
    }
}
