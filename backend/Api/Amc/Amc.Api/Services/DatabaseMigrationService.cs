...
public class DatabaseMigrationService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        AmcDbContext db = scope.ServiceProvider.GetRequiredService<AmcDbContext>();
        ...
        while (retries > 0)
        {
            try
            {
                _logger.LogInformation("Migrating Amc database...");
                await db.Database.MigrateAsync(cancellationToken);
                _logger.LogInformation("Amc database migration complete.");
                break;
            }
