...
/// <summary>
/// Approval chains for spend money payments (TK-101).
///
/// <b>Submitting is an edit</b> (<c>banking.edit</c>). <b>Acting and reading need
/// only <c>banking.view</c></b>, because being the level's approver is the
...
public sealed class SpendMoneyApprovalsController : ControllerBase
{
    private const ApprovalRequestKind Kind = ApprovalRequestKind.SpendMoney;

    private readonly AccountingApprovalService _approvals;

    public SpendMoneyApprovalsController(AccountingApprovalService approvals) => _approvals = approvals;
    ...
    /// <summary>Payments waiting on the signed-in user, for the approvals inbox (TK-103).</summary>
