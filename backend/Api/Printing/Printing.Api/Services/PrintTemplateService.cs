...
/// <summary>
/// The print template master, and the render endpoint's resolution of which
/// layout a document prints with. Branch-scoped throughout — the query filter on
/// PrintingDbContext supplies CustomerId and OrgId, so no method here repeats
...
public sealed class PrintTemplateService
{
    ...
    public PrintTemplateService(PrintingDbContext db, PrintRenderer renderer)
    {
        _db = db;
        _renderer = renderer;
    }
        ...
        long id, PreviewPrintTemplateRequest request, CancellationToken ct)
    {
        ...
        {
            // Printing cannot read sal, pur or acc. The document's own service
            // builds the payload and posts it to api/print/render.
    ...
    /// <summary>
    /// Renders a document the calling service pushed, with the layout this
    /// branch resolves for it.
    ///
    /// The payload is the caller's; the only thing read here is the branch's own
    /// template, through the query filter. Nothing is written.
    /// </summary>
    public async Task<PrintTemplateResult<RenderPrintResponse>> RenderAsync(
        RenderPrintRequest request, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(request);

        if (!DocumentTypeCatalog.IsPrintable(request.DocumentTypeCode))
        {
            return PrintTemplateResult<RenderPrintResponse>.Fail(PrintTemplateOutcome.InvalidDocumentType);
        }

        PrintTemplateResolution layout = await ResolveAsync(
            request.DocumentTypeCode, request.PrintTemplateId, ct);
        ...
        return PrintTemplateResult<RenderPrintResponse>.Ok(new RenderPrintResponse
        {
            ...
            PageCount = rendered.PageCount,
            UnknownTags = rendered.UnknownTags,
            PrintTemplateId = layout.PrintTemplateId,
