...
/// <summary>
/// Writes Admission's master data for a branch (S2, TK-62). Called by Master's
...
public sealed class InternalSeedController : ControllerBase
{
    ...
    public async Task<IActionResult> SeedOrganization([FromBody] SeedOrganizationRequest request, CancellationToken ct)
    {
        ...
        {
            Seeded = await _services.GetRequiredService<AdmissionSeeder>().SeedForOrganizationAsync(request.OrgId, ct),
        };

        _log.LogInformation("Seeded Admission for organization {OrgId}: {@Seeded}", request.OrgId, response.Seeded);
        return Ok(response);
    }
}
