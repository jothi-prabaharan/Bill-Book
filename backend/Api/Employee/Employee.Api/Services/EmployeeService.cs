...
public sealed class EmployeeService
{
    ...
    public EmployeeService(EmployeeDbContext db, INumberGenerator numbers, ICallerPermissions caller, TimeProvider clock)
    {
        _db = db;
        _numbers = numbers;
        _caller = caller;
        ...
        string? search, long? departmentId, EmployeeStatus? status, int page, int pageSize, CancellationToken ct)
    {
        ...
        IQueryable<EmployeeRecord> query = _db.Employees.AsNoTracking();
    ...
    {
        EmployeeRecord? e = await LoadAsync(employeeId, tracking: false, ct);
        if (e is null)
        {
            return null;
        }
    ...
    public async Task<EmployeeResult> CreateAsync(SaveEmployeeRequest request, CancellationToken ct)
    {
        ...
        NumberAllocation code = await _numbers.NextAsync(EmployeeSeed.EmployeeSeriesCode, request.JoiningDate, ct);

        var employee = new EmployeeRecord { EmployeeCode = code.Code, NoticePeriodDays = notice };
    ...
    {
        EmployeeRecord? employee = await LoadAsync(employeeId, tracking: true, ct);
        if (employee is null)
        {
    ...
    private async Task<EmploymentChangeKind?> ChangeOfAsync(EmployeeRecord e, SaveEmployeeRequest r, CancellationToken ct)
    {
    ...
    private static void Apply(EmployeeRecord e, SaveEmployeeRequest r, string? pan, string? aadhaar)
    {
    ...
    private static void ReplaceChildren(EmployeeRecord e, SaveEmployeeRequest r, IReadOnlyDictionary<long, string> storedAccounts)
    {
    ...
    private static EmploymentHistory History(EmployeeRecord e, EmploymentChangeKind kind, DateOnly on, string? remarks) => new()
    {
        EmployeeId = e.EmployeeId,
        EffectiveDate = on,
        ChangeKind = kind,
    ...
    private bool MaySeeSensitive(EmployeeRecord e) =>
        _caller.Has(SalaryPermission) || (e.UserId is Guid own && own == _caller.UserId);

    private async Task<EmployeeRecord?> LoadAsync(long employeeId, bool tracking, CancellationToken ct)
    {
        IQueryable<EmployeeRecord> query = _db.Employees
