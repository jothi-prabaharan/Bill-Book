...
public sealed class OrganisationController : ControllerBase
{
    ...
    private IActionResult Answer(OrgMasterResult result) => result.Outcome switch
    {
        ...
        OrgMasterOutcome.DuplicateCode => Conflict(new EmployeeMessage("Another entry in this branch already uses that code.")),
        _ => UnprocessableEntity(new EmployeeMessage("Choose a head or parent from this branch, and never a department under itself.")),
    };
}
