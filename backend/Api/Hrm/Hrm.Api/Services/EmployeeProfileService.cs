...
public sealed class EmployeeProfileService
{
    ...
    public EmployeeProfileService(HrmDbContext db, INumberGenerator numbers)
    {
        _db = db;
        _numbers = numbers;
    }
    ...
    public async Task<OnboardEmployeeResult> OnboardAsync(OnboardEmployeeRequest request, CancellationToken ct)
    {
        ...
        var alloc = await _numbers.NextAsync(HrmSeed.EmployeeSeriesCode, request.JoiningDate, ct);
        ...
        var employee = new Hrm.Entity.TableEntities.Employee
        {
