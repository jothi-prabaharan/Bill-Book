...
public sealed class NoticesController : ControllerBase
{
    ...
    private IActionResult Answer(NoticeResult result) => result.Outcome switch
    {
        ...
        NoticeOutcome.ExpiresBeforePublish => UnprocessableEntity(new HrmMessage("The expiry date cannot be before the publish date.")),
        _ => UnprocessableEntity(new HrmMessage("Choose the department, location or grade the announcement is for.")),
    };
}
