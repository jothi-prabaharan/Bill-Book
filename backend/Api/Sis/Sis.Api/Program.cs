...
// Sis (S1, TK-61): academic years, classes, sections, subjects, students with guardians, enrolments and exams. Schema sis, port 4515. The scaffold is Hrm's.
...
builder.Services.AddDbContext<SisDbContext>((sp, options) =>
{
    options.UseNpgsql(
        sp.GetRequiredService<ITenantDatabaseResolver>().GetConnectionString(sp.GetService<ITenantContext>()?.CustomerId),
        npgsql => npgsql.MigrationsHistoryTable("__EFMigrationsHistory", "sis"));
...
// into sis.ErrorLogs (hard rules 13 and 14).
builder.Services.AddBillBookReliability<SisDbContext>();
...
builder.Services.AddScoped<INumberGenerator>(sp => new NumberGenerator(
    sp.GetRequiredService<SisDbContext>(),
    sp.GetRequiredService<IOptions<NumberingOptions>>(),
    sp.GetRequiredService<IFinancialYearProvider>()));

// Guardians are Master's contacts, checked before they are stored.
