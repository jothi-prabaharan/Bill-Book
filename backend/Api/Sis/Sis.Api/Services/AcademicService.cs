...
public sealed class AcademicService
{
    ...
    public AcademicService(SisDbContext db) => _db = db;
    ...
    public async Task<SisResult> SaveYearAsync(long? id, SaveAcademicYearRequest request, CancellationToken ct)
    {
        if (request.EndDate <= request.StartDate)
        {
            return SisResult.Fail(SisOutcome.YearRule, "A school year must end after it starts.");
        }

        string code = request.Code.Trim();
        if (await _db.AcademicYears.AnyAsync(y => y.AcademicYearId != (id ?? 0) && y.Code == code, ct))
        {
            return SisResult.Fail(SisOutcome.Duplicate);
        }
        ...
        {
            return SisResult.Fail(SisOutcome.YearRule, "School years in a branch cannot overlap.");
        }
        ...
        {
            return SisResult.Fail(SisOutcome.YearRule, "The current school year cannot be closed.");
        }
        ...
        return SisResult.Ok(year.AcademicYearId);
    }
    ...
    public async Task<SisResult> SaveClassAsync(long? id, SaveSchoolClassRequest request, CancellationToken ct)
    {
        string code = request.Code.Trim();
        ...
        await _db.SaveChangesAsync(ct);
        return SisResult.Ok(row.SchoolClassId);
    }
    ...
    public async Task<SisResult> SaveSectionAsync(long? id, SaveSectionRequest request, CancellationToken ct)
    {
        ...
        {
            return SisResult.Fail(SisOutcome.Invalid, "Choose a school year and a class from this branch.");
        }
        ...
        {
            return SisResult.Fail(SisOutcome.YearRule, "A section with students cannot move to another year or class.");
        }
        ...
        {
            return SisResult.Fail(SisOutcome.SectionFull, "The section already has more students than that capacity.");
        }
        ...
        await _db.SaveChangesAsync(ct);
        return SisResult.Ok(row.SectionId);
    }
    ...
    public async Task<SisResult> SaveSubjectAsync(long? id, SaveSubjectRequest request, CancellationToken ct)
    {
        string code = request.Code.Trim();
        ...
        await _db.SaveChangesAsync(ct);
        return SisResult.Ok(row.SubjectId);
    }
}
