...
public class DatabaseMigrationService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        SisDbContext db = scope.ServiceProvider.GetRequiredService<SisDbContext>();
        ...
        while (retries > 0)
        {
            try
            {
                _logger.LogInformation("Migrating Sis database...");
                await db.Database.MigrateAsync(cancellationToken);
                _logger.LogInformation("Sis database migration complete.");
                break;
            }
