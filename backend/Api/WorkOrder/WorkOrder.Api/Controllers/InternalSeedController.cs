...
/// <summary>
/// Writes WorkOrder's master data for a branch (S6, TK-66). Called by Master's
...
public sealed class InternalSeedController : ControllerBase
{
    ...
    public async Task<IActionResult> SeedOrganization([FromBody] SeedOrganizationRequest request, CancellationToken ct)
    {
        ...
        {
            Seeded = await _services.GetRequiredService<WorkOrderSeeder>().SeedForOrganizationAsync(request.OrgId, ct),
        };

        _log.LogInformation("Seeded WorkOrder for organization {OrgId}: {@Seeded}", request.OrgId, response.Seeded);
        return Ok(response);
    }
}
