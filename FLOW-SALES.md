...
The series stays gapless because **no document row is ever deleted**: an abandoned draft is `Cancelled` and keeps its number, which makes the number answerable rather than missing. Consecutive numbering on an Indian invoice is statutory, not a preference.

### Statuses

**Draft → ReadyToPost → Posted → Void**, and **Cancelled** for one abandoned before it ever posted.
