...
The work queue is `docs/TASKS.md`. Read its section 0 before starting anything: it holds the claim protocol, the Senior/Junior levels and the lanes that let several agents work in parallel. The reality check above applies to the queue as well: verify a card's claims against the code before starting it, and correct the card in your claim commit if it is wrong.
