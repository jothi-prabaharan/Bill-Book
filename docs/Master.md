...
- [x] **5.20 — Collapse one database per customer to one shared tenant database, with `apps/admin` to manage it** — *repository owner's decision, 25 August 2026*
  Reverses a decision `CLAUDE.md` had documented as settled: each Customer owned a physical database, created at signup by `CREATE DATABASE`. A thousand customers each waiting on their own database, template clone and Key Vault entry does not suit an operator-driven admin screen creating them one at a time, so the model moved to the one already proven at the branch level — one shared database, isolated by an id checked on every query — one level up.
  `CustomerId` added to every `OrgScopedEntity` table across all seven services, checked alongside `OrgId` in the same query filter, RLS policy and `set_config` call. Defence in depth rather than a replacement: `OrgId` was already globally unique on its own. `ITenantConnectionResolver`, the tenant directory, `CustomerDatabase`, `ProvisioningWorker` and the `CustomerProvisioned` event are gone; a service opens the one tenant database at startup, the same way it always opened the one master database. Signup and admin-initiated creation both seed synchronously in the request now, since there is no database left to create and wait on.
  **Verifying it surfaced real, pre-existing bugs**, in the same spirit `CLAUDE.md`'s own caveats describe for `sal`: `con.ApiClients` had no RLS at all; `acc.NumberingSeries` (an `OrgScopedEntity`) had never had one; `inv.PriceLists`/`PriceListItems` had a policy that was never `ENABLE`d; `Sales.Api` never registered `IFileStorage`, so `InvoiceService` could not have resolved at runtime. All fixed, each covered by a query-filter test asserting the composite filter and RLS coverage over the whole model — the same shape of test that caught `sal`'s gaps, now written before the gap rather than after.
  **Browser-testing the admin portal before calling the UI stage done found a worse one**: `DatabaseMigrationService` still read the connection string under its pre-rename key, so Master could not start at all — every deployment, not a corner case. The same stale key was sitting inert in six `ConfigurationSecretStore` implementations and `InMemorySecretStore`, and the Gateway had never been given a route for the new `/api/admin/**` prefix, so the API would have been unreachable through it regardless. All fixed; see `CLAUDE.md`'s dated note in the Tenancy section for the full account.
  **`apps/admin` is built and unreachable by anyone.** Customer list with status, admin-initiated creation, retry for one stuck mid-seed, read-only branch view — verified end to end against a live backend (created a customer, watched it seed across four services, forced a real `Failed` by stopping one mid-signup, retried it back to `Trial`), but every action needs `platform.*` and nothing seeds that permission onto any account. Deliberately not fixed here: `Role` rows are shared system rows rather than per-customer copies, so granting `platform.*` to a tenant role would grant platform access to that role's holders across every customer, not just one. How an operator's own account is meant to acquire it is an owner decision, not a code one — tracked in `CLAUDE.md`'s Undecided section.
  *Done when*: two customers seeded into the same database, and a session scoped to one's `CustomerId`/`OrgId` returns zero rows for the other's, in both directions, as a real test rather than a manual check — and a platform operator account exists that can actually sign in to `apps/admin`. The first is done (`*QueryFilterTests` per service). The second is not: that is the open item above.
...
- [`Sales.md`](./Sales.md) — quote → order → invoice → receipt, plus POS and the credit note
- [`Purchase.md`](./Purchase.md) — order → receipt → bill → payment, plus the debit note
- [`Inventory.md`](./Inventory.md) — the pool, the guarded move, the two queues, costing and recosting. **Built, so it describes running code** rather than an intention, unlike the other two
