...
- [x] **Period locks — closing the books, one date per branch per role** *(owner request, not an original box)*
  ...
  **A user holds exactly one role per branch** (`mst.UserOrganizationRole` is read singly), so a per-role lock resolves to one date per user with nothing to reconcile. That required a **`role_id` claim on the access token**, which did not exist — the token carried `permission[]` only, and permissions cannot say which role granted them. `ICurrentUser.RoleId` follows it through every service.
...
- [x] **T6.1 — the money-document schema** — payments, receipts and transfers, with lines that allocate to documents. Written as `bnk.*`; it is `acc.*` now that Banking and Accounting are one service.
  **Done — as three tables, not one.** Built first as a single discriminated pair following T2.1's recommendation for `sal`, then **split at the owner's direction**: `acc.SpendMoney` + `acc.SpendMoneyDetails`, `acc.ReceiveMoney` + `acc.ReceiveMoneyDetails`, and `acc.TransferMoney`.
