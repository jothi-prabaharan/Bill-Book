...
### `sal.*` and `pur.*` — sales and purchase documents 🔨

**Moved.** The columns, indexes, check constraints, decisions and open questions for all 27 tables now live in [`SALES.md`](./SALES.md) and [`PURCHASE.md`](./PURCHASE.md), one file per module, alongside their task lists and the reasoning behind each choice. They are not repeated here — this entry is the pointer, not a summary.

| | |
|---|---|
| [`SALES.md`](./SALES.md) | `Quotes`, `SalesOrders`, `DeliveryChallans`, `Invoices` (also POS), `CreditNotes`, each with details and tax rows, plus `sal.SalesRegister` |
| [`PURCHASE.md`](./PURCHASE.md) | `PurchaseOrders`, `GoodsReceipts`, `Bills`, `DebitNotes`, each with details and tax rows |
