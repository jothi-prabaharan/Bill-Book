...
- [ ] **S · R0.4 ★ — the generic query engine:** *half done.* Built: `ReportColumn`, `ReportParameter`, `ReportQueryBuilder` (every operator, multi-key sort, paging), `IReportSource` + `ReportSource<TRow>` with execution, count and grand totals, and 23 tests against an in-memory sequence. **Remaining: group footers, `ReportCatalogService`, `ReportCatalogValidator`.**
  - **Group footers need a decision first.** Grouping in SQL means `GroupBy` on a key whose type varies per column, and the translatable way to do that generically is to require **groupable columns to be text** — a report wanting to group by account type exposes the type's *name* as a column rather than its id. The alternative is a per-shape key DTO and reflection, which is more code and more ways to fail at translation. Recommend the text rule.
