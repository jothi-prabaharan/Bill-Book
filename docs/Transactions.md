...
- [ ] **T0.2 — There is no tax determination anywhere**
  ...
  **Written, and verified on both sides since.** `shared-fixtures/tax-fixture.json` is read by `GstCalculatorFixtureTests` in C# and `tax-fixture.spec.ts` in TypeScript, so a divergence between the two implementations is a failing test rather than a wrong GST return. Four of the five *Done when* clauses are covered there — per line, per document, the line sum equalling the header, and place-of-supply resolution.
  **The fifth is not.** "A contact whose GSTIN state code contradicts its place of supply is refused rather than posted" is implemented — `PlaceOfSupply.Resolve` returns the refusal and `SalesOrderService` acts on it — but nothing asserts it, so it is the one clause holding this box open.
  `Shared.Kernel.Tax` holds the pure calculator (`GstCalculator`), the place-of-supply resolver (`PlaceOfSupply`) and the cached rate client (`ITaxRateProvider`); Accounting serves `GET internal/tax/rates?on={date}`. Sales registers the provider; Purchase registers the same one when it lands.
...
- [ ] **T0.3 — No document numbering series exist**
  ...
  **Built and partly proven.** Every service seeds its own document series and allocates at creation: `QTE`, `SOR`, `INV`, `CRN`, `POS` in Sales, the purchase codes in Purchase, `STA` in Inventory, `JRN`/`OPB` in Accounting and the three money codes beside them. The third clause is covered — `SalesOrderServiceTests` voids a draft and finds the row and its number still there.
  **The first two are not tested.** Nothing asserts that two concurrent creates take consecutive numbers, or that a create which fails gives its number back. Both are properties of `NumberGenerator`'s guarded `ExecuteUpdate` joining the caller's transaction, which is exactly the kind of claim that is either true or catastrophically not, and is worth a test that runs two creates against one series.
...
- [ ] **T0.4 — One lifecycle, the same for every type**
  ...
  **The withdrawal is half-covered now.** `Voiding_a_posted_invoice_withdraws_ledger_rows_and_cleans_sales_register` proves a void withdraws exactly its own ledger rows, and `Voiding_is_blocked_if_downstream_credit_note_exists` proves it is refused once something points at the document. What no test asserts is the other half of that clause — that the void also releases exactly its own reservation — even though `Posting_invoice_with_sales_order_releases_reservation` shows the release path works when an invoice issues. That single assertion is what is left.
  **The rule and the permission are written.** `Shared.Kernel.Documents.DocumentLifecycle` is the whole table in one place — pure, so whether anything downstream points at the document is passed in rather than queried, and testable without a server. Four of the five *Done when* clauses are covered by `DocumentLifecycleTests`; the fifth — that a void withdraws exactly its own ledger rows and releases exactly its own reservation — needs a document that posts, so it belongs to T3.1 and is named there rather than ticked here.
