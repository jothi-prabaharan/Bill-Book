...
- **Run `npm run check` in `frontend/` and `dotnet build && dotnet test` in `backend/` before claiming anything works.** As of 4 September 2026 the frontend chain is fully green — lint across 23 projects, typecheck with zero errors, **533 tests** (plus 3 skipped) and **all five app builds** under `strictTemplates`. The backend builds clean and runs **1,036 tests, none skipped** — **1,032 pass and 4 fail**, given a PostgreSQL and clean test databases. The four are `Reporting.Api.Tests.ReportLayerCertificationTests`, which counts report sources against an expected 41 that the last seven reports moved past; they were **already red on `origin/main`** and are unrelated to anything since. **`ADMIN_TEST_DB` and `CONTACTS_TEST_DB` are both needed for Master** — the suite has two fixtures now, and a missing database is a wholesale skip rather than a failure. This figure has been 845 and then 969 within a week; count it rather than copy it. **`ADMIN_TEST_DB` and `CONTACTS_TEST_DB` are both needed for Master** — the suite has two fixtures now, and a missing database is a wholesale skip rather than a failure. The old figure here was 845; it was counted before Reporting's last stage and the print templates, and re-running is cheaper than trusting it. Both figures were re-run from dropped databases on 4 September, so the migration chain applies from zero and the RLS assertions run against a schema built by the migrations rather than one left over. Each suite points at its own — `SALES_TEST_DB`, `PURCHASE_TEST_DB`, `ACCOUNTING_TEST_DB`, `INVENTORY_TEST_DB`, `BANKING_TEST_DB`, `REPORTING_TEST_DB` — and **a stale one is the usual cause of a suite skipping wholesale**, because the fixture migrates into it and a half-migrated database throws. Drop and recreate rather than reusing. In a fresh container there is no server running: `service postgresql start`, then `ALTER USER postgres PASSWORD '123'` to match the default connection strings.
