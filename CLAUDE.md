...
**Designed, no phase, nothing built — HRMS & Payroll first, then School** (owner's decision, 23 September 2026) — **HRMS & Payroll** (`apps/hrms`; `hrm` `pay`), whose stage H0 builds the shared platform work: an `App` flag (`RetailErp` / `School` / `Hrms`) on `mst` roles, permissions and menus, a per-app shell, and the shard-model fix. Then **School management & maintenance** (`apps/school`; eight services, one per schema: `sis` `adm` `att` `fee` `fac` `wrk` `ppm` `amc`), which depends on H0 and on HRMS's employee master. Both are separate apps on `libs/app-shell` and the shared libs. Designs are the `HRMS & Payroll` and `School` sections at the end of [`docs/Modules.md`](./docs/Modules.md). The service count above stays **7** until one of them carries traffic.
