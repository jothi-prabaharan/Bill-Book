...
- **Notification.Worker is a stub that runs; RateSync.Worker is empty.** Notification.Worker hosts `PaymentReminderWorker`, which writes a `ReminderLog` row per overdue invoice every 24 hours and **sends no email** — it also reads across tenants with `IgnoreQueryFilters()`, so it will see nothing once RLS is back (TK-72 in `docs/TASKS.md`). RateSync.Worker is a `Program.cs` and nothing else. Email currently sends from Master (`SmtpEmailSender` + an in-process `EmailQueue`), not from a worker
