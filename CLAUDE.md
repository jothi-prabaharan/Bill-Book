...
- **Public signup was broken on `main` from the tenancy reversal until 4 September 2026, and is fixed.** `Customer.DatabaseName` was `[Required]` over a NOT NULL column that `SignupService` never set, so every `POST /api/customers/signup` died on a not-null violation and **there was no way to create a customer through the product at all**. The column was the last piece of the one-database-per-customer model reversed on 25 August: nothing read it or wrote it afterwards, so dropping it finished a decision already taken rather than making a new one, which is what made it a repair and not the tenancy question it looked like.
