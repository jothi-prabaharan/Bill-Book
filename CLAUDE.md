...
- **Notification.Worker is a stub that runs; RateSync.Worker is empty.** Notification.Worker hosts `PaymentReminderWorker`, which writes a `ReminderLog` row per overdue invoice every 24 hours and **sends no email** — it also reads across tenants with `IgnoreQueryFilters()`, so it will see nothing once RLS is back (TK-20 in `docs/TASKS.md`). RateSync.Worker is a `Program.cs` and nothing else. Email currently sends from Master (`SmtpEmailSender` + an in-process `EmailQueue`), not from a worker
...
- **Purchase's numbering series never reach a new branch.** Every service that owns a series has a `SeedData/NumberingSeriesSeed.cs`: Accounting (`JRN`, `OPB`, `SPM`, `RCM`, `TRM`, plus the master-code series), Inventory (`STA`), Sales (`QTE`, `SOR`, `DLC`, `INV`, `CRN`, `POS`) and Purchase (`POR`, `GRN`, `BIL`, `DBN`). Purchase's is called by its own `PurchaseSeeder` behind `internal/seed/organization`, but `TenantSeeder` seeds only Accounting, Inventory and Sales, and the startup bootstrap in `DatabaseMigrationService` skips it too — so a new branch cannot number a purchase document. Reporting's seed endpoint is missed the same way. TK-01 in `docs/TASKS.md`
...
- **`FORCE ROW LEVEL SECURITY` was enforced everywhere and asserted nowhere.** Each schema had a test reading `pg_tables.rowsecurity`, which says RLS is switched on. It does not say a policy exists, and it does not say FORCE is set — and **without FORCE, RLS does not apply to the table's owner, which is the role the application connects as**. Every policy in the product would have been inert, leaving the EF query filter as the only guard, which is the single point of failure having both is meant to avoid. One shared `tests/Shared/RlsAudit.cs`, linked into all seven suites, now asks the catalog all three questions of every table in a schema, with exemptions named as arguments: `rpt.ReportMasters` and `rpt.ReportColumns` hold the imported `reports.json` specification and carry no tenant column at all, so there is nothing in them to scope.
  ...
  **Being restored schema by schema (TK-71, TK-72, TK-73, TK-74, TK-02, TK-03, TK-04 in `docs/TASKS.md`).** `acc` is back as of
  ...
  **The title of this bullet was false, and how it became false is the point.** RLS was not
  ...
  September 2026, and the hand-written RLS blocks were in the chains it replaced.

  **It reads as enforced because the developer databases predate the squash.** They still carry
  policies named `{table}_tenant_isolation`, a string that is in no source file any more. Every
  RLS assertion passes against those and fails against a database built from the current chain.
  Dropped and re-run on 18 September 2026: **1,042 tests, 1,022 pass, 20 fail, 0 skip** — 16 of
  the 20 are RLS, two in each of `acc`, `con`, `cus`, `inv`, `pur`, `rpt` and `sal`; the other 4
  are the known `ReportLayerCertificationTests`. `Printing.Api.Tests` passes 5 of 5 because the
  `prt` migration writes its own block, which is the shape to copy.

  So the paragraph above describes the guard correctly and the product does not have it: the EF
  query filter is currently the only thing between one branch's rows and another's. Nothing leaks
  while the filter holds, which it does — but `IgnoreQueryFilters`, a raw command, or a context
  that omits `base.OnModelCreating` (`SalesDbContext` did exactly that) would meet no second
  guard. Restoring it is a migration per service over roughly ninety tables and has not been done.
  The policy text is recoverable verbatim: `SELECT tablename, policyname, qual FROM pg_policies
  WHERE schemaname = 'cus'` against any un-dropped developer database. See `docs/Modules.md`.

  **The lesson underneath both halves of this bullet is the same one twice.** The first half is
  about a test that asked one of three questions; the second is about the right test asked of the
  wrong database. A green RLS suite means nothing until the database under it was built from zero
  and one policy has been dropped by hand to watch it go red.
...
*What is still missing: **capitalising and disposing post nothing.** A bill's capital line posts to one shared Fixed Asset account and creates no register row; capitalise is a hand-called endpoint that moves nothing in the ledger; dispose records the proceeds and posts nothing. Both postings are decided (24 September 2026) and are TK-12 in `docs/TASKS.md`: capitalising reclassifies the bill's shared Fixed Asset account to the category's account (D-19), and a disposal either names the bank or cash account the proceeds landed in or is raised as a sales invoice to the buyer (D-20). The controller also writes through `_db` directly and breaks several house rules (TK-11), and the two pages under `accounting-ui/src/lib/fixed-assets/` are routed from nowhere. The old T10 stage file, `TRANSACTIONS-ACCOUNTING-BANKING.md`, no longer exists.*

*The consequence to carry: **the opening balance still cannot migrate a fixed asset.** The register exists, and D-19 decides the posting — a migrated asset debits its category's account against Opening Balance Equity — but until TK-12 builds it one still comes across as a plain account balance.*
