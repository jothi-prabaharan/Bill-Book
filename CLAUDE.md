...
**Designed, no phase, nothing built** — **School management & maintenance** (`apps/school`; eight services, one per schema: `sis` `adm` `att` `fee` `fac` `wrk` `ppm` `amc`) and **HRMS & Payroll** (`apps/hrms`; `hrm` `pay`). Both are separate apps on `libs/app-shell` and the shared libs, and both depend on an `App` flag (`RetailErp` / `School` / `Hrms`) on `mst` roles, permissions and menus. Designs are the `School` and `HRMS & Payroll` sections at the end of [`docs/Modules.md`](./docs/Modules.md). The service count above stays **7** until one of them carries traffic.
