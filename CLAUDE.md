...
- **Sales past the quote and the sales order.** The sixteen `sal` tables, the three document base classes and all five services and controllers are written; the quote (T2.1) and the sales order (T2.2) also have their pages, their docs and their tests. **Invoice, delivery challan and credit note have a controller and a scaffold page but no verified path** — until 21 August none of them could save a document at all (see below), so "written" has never been "works" for those three. The invoice is also blocked on `LedgerClient` sending a single `Amount` where Accounting takes `DebitAmount`/`CreditAmount`, across thirteen leg sites
