...
- **Run `npm run check` in `frontend/` and `dotnet build && dotnet test` in `backend/` before claiming anything works.** As of 2 September 2026 the frontend chain is fully green — lint across 23 projects, typecheck with zero errors, **474 tests** and **all five app builds**. The backend builds clean and runs **756 tests with none skipped**, given a PostgreSQL and clean test databases; **755 pass and one does not** — `Accounting.Api.Tests.ReconciliationMatchingTests.GetSuggestedMatches_FindsExactAmountWithinThreeDays`, which arrived with bank reconciliation and is a test-data fault rather than a fault in the matcher. Each suite points at its own — `SALES_TEST_DB`, `PURCHASE_TEST_DB`, `ACCOUNTING_TEST_DB`, `INVENTORY_TEST_DB`, `BANKING_TEST_DB`, `REPORTING_TEST_DB` — and **a stale one is the usual cause of a suite skipping wholesale**, because the fixture migrates into it and a half-migrated database throws. Drop and recreate rather than reusing. In a fresh container there is no server running: `service postgresql start`, then `ALTER USER postgres PASSWORD '123'` to match the default connection strings.
...
- **Every endpoint is behind a credential and a permission** — true again as of 2 September 2026, having not been: `ApiClientsController`, `CreditNotesController` and `PriceListsController` carried no permission attribute, and `InternalApiKeysController` was anonymous with no shared-key guard. `Customer.Api.Tests.PermissionModuleTests` now fails the build if a controller demands a module the catalogue never seeds, which is how Leads and Tickets came to be locked to every role including Owner. Services default-deny; the exceptions are sign-in, signup and the country/state lists the signup form needs. `internal/` routes take a shared key instead of a token.
