...
Schema, API and page all exist for these. Task tracking lives in the per-module files under [`docs/`](./docs/) — **there is no `Modules.md`**, though older notes name one. This is the shape of the thing, not the to-do list.
...
**Designed, no phase, nothing built — three apps, one customer** (owner's decisions, 23 September 2026). RetailErp, **HRMS & Payroll** and **School** are each sold on their own, and one customer may buy any or all of them: one set of branches and users across every app, **one licence per app**, per-app sign-in (an `app` claim, that app's licence claims and only that app's permissions), an app switcher, and a public signup with a 14-day trial per app. That platform work is stage H0 and is built first. Then **HRMS & Payroll** (`apps/hrms`; six services, one per schema: `hrm` `tla` `pay` `rec` `prf` `clm` — core HR, time and leave, payroll with Indian statutory and income tax, recruitment, performance, expense claims), then **School management & maintenance** (`apps/school`; `sis` `adm` `att` `fee` `fac` `wrk` `ppm` `amc`), which depends on H0 and on HRMS's employee master. Designs are the `One customer, many applications`, `HRMS & Payroll` and `School` sections at the end of [`docs/Modules.md`](./docs/Modules.md). The service count above stays **7** until one of them carries traffic.
