...
- BCrypt work factor 12; all tokens stored **hashed**. **Refresh-token rotation is designed but not built**: tokens are minted and hashed at sign-in, and no endpoint anywhere reads or validates a presented one — the frontend stores a refresh token and never sends it
...
- ~~**Customer, Purchase, Reporting**~~ — **all three are built; this line was stale and is kept only so nobody trusts a copy of it.** Purchase has 12 tables, 6 controllers / 33 actions and 45 tests across PO → GRN → Bill → Debit Note. Reporting has 41 reports wired end to end, an OpenXML Excel writer, and 344 tests. Customer has Lead/Ticket/TicketMessage, 2 controllers / 12 actions and 5 UI components, wired into `apps/web`; only its seed data (stage C4) is genuinely not started
...
- **Public signup is broken on `main`, and has been since the tenancy reversal.** `Customer.DatabaseName` is `[Required]` with a NOT NULL column, and `SignupService` never sets it — so every `POST /api/customers/signup` dies on a not-null violation. The column is vestigial: it belonged to the one-database-per-customer model reversed on 25 August, and while `CustomerDatabase`, `ITenantConnectionResolver` and the rest went, this column and its `[Required]` stayed. **Nothing catches it**, because no test signs a customer up through the API and the seeded test fixtures build their rows directly. Found on 4 September by trying to sign in to run the app, and deliberately not fixed here: drop the column, make it nullable, or write a constant is a tenancy decision, not a passing repair. Until it is made, **there is no way to create a customer through the product**, which is also why any "run the app end to end" attempt stops at the front door.
...
- **Run `npm run check` in `frontend/` and `dotnet build && dotnet test` in `backend/` before claiming anything works.** As of 4 September 2026 the frontend chain is fully green — lint across 23 projects, typecheck with zero errors, **520 tests** (plus 3 skipped) and **all five app builds** under `strictTemplates`. The backend builds clean and runs **763 tests, all passing, none skipped**, given a PostgreSQL and clean test databases. Each suite points at its own — `SALES_TEST_DB`, `PURCHASE_TEST_DB`, `ACCOUNTING_TEST_DB`, `INVENTORY_TEST_DB`, `BANKING_TEST_DB`, `REPORTING_TEST_DB` — and **a stale one is the usual cause of a suite skipping wholesale**, because the fixture migrates into it and a half-migrated database throws. Drop and recreate rather than reusing. In a fresh container there is no server running: `service postgresql start`, then `ALTER USER postgres PASSWORD '123'` to match the default connection strings.
...
*Report Excel/CSV export moved Phase 1 → Phase 2 on 24 August 2026, by decision, as a roadmap label only — the Excel half is built and shipped — `ExcelReportWriter` lives in `Reporting.Api` today, proven by `ExcelReportWriterTests`. **CSV export does not exist**: `ExportFormat` carries only `Xlsx` and `Pdf`, and there is no CSV path anywhere in the service. This records where the next round of export work sits in priority, not that the existing export stopped working.*
