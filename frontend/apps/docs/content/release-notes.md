...
- **Branches** — the places you trade from, with their own code, address and, where they hold one, their own GSTIN. Exactly one is the head office, and every new organization arrives with one already created from its own details. Numbering series and warehouses now pick a branch from a list instead of taking a number nobody could look up, and a series that includes the branch code fills it in from the branch. Branches are a reporting and numbering dimension only — stock stays one shared pool across all of them.
