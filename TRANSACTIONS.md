...
- [ ] **T0.3 — No document numbering series exist**
  ...
  *Done when*: two invoices created concurrently take consecutive numbers, a create that fails takes none, and an abandoned draft is `Cancelled` with its number still accounted for rather than deleted.

- [ ] **T0.4 — One lifecycle, the same for every type**
  **Draft → ReadyToPost → Posted → Void**, with **Cancelled** for a document abandoned before it ever posted, plus Reversed for journals.
  ...
  `Cancelled` and `Void` are deliberately separate. One says the document never entered the books, the other that it did and was taken back out — and only the second has ledger rows to withdraw and stock to put back.
