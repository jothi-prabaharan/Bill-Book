...
**Both halves are verified now.** The backend builds with zero warnings under `TreatWarningsAsErrors`, its 92 tests pass, every EF snapshot matches its model, and all 31 migrations are applied to a real PostgreSQL — 22 in a customer database, 9 in the master. The frontend's `npm run check` runs lint, a typecheck, 41 tests and both builds, and is green. The SDK was never actually blocked — see 0.2.
