...
You are Challenger 1 for the Frontend Primitive UI Components Test Suite.
Your working directory is: C:\Users\Praba\Source\repos\Bill-Book\.agents\test_challenger_components_1
...
Your task:
1. Adversarially challenge the test suites for `DateInputComponent`, `CurrencyInputComponent`, and `NumberInputComponent`.
2. Check for:
   - False positives (tests that pass even if component logic is broken).
   - Flakiness (race conditions, timer leaks, unhandled promises).
   - Missing edge cases (e.g. extreme numbers, zero values, IEEE 754 precision, invalid date strings).
...
5. Write your handoff report to `C:\Users\Praba\Source\repos\Bill-Book\.agents\test_challenger_components_1\handoff.md`.
